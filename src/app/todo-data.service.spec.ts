...
describe('TodoDataService', () => {
  ...
  describe('#getAllTodos()', () => {
    it('should be an empty array by default', inject([TodoDataService], (service: TodoDataService) => {
  ...
  describe('#updateTodoById(id, values)', () => {
    it('should return todo with the corresponding id and updated data', inject([TodoDataService], (service: TodoDataService) => {
      const todo = new Todo({title: 'Hello 1', completed: false});
      service.addTodo(todo);
      const updatedTodo = service.updateTodoById(1, {title: 'new title'});
      expect(updatedTodo.title).toEqual('new title');
    }));
    it('should return if todo is not found', inject([TodoDataService], (service: TodoDataService) => {
      const todo = new Todo({title: 'Hello 1', completed: false});
      service.addTodo(todo);
  ...
  describe('#toggleTodoComplete(todo)', () => {
    it('should return the updated todo with inverser complete status', inject([TodoDataService], (service: TodoDataService) => {
